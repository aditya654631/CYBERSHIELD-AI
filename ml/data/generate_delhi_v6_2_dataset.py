"""
CyberShield AI — Delhi V6.2 Controlled Synthetic Dataset Generator
Phase A.4L: One-Shot Implementation of V6.2 Conditional Geography & Cluster Diversity Dataset
...
1. Conditional Concentration + Marginal Cluster Diversity:
   - Eliminates global hub collapse (Connaught Place restricted to NEW_DELHI, Seelampur restricted to NORTH_EAST).
   - District-local functional role portfolios (Commercial, Transport, ATM, Branch, Retail, Secondary Corridor).
   - Sub-prototypes and fraud types map to functional roles across districts, not specific cluster IDs.
2. Cross-District Route Coherence:
   - 11 new pre-event route and consensus features (downstream_route_consistency, route_entropy, etc.).
   - Multi-channel Stage 1 district scoring supporting cross-district learnability headroom (>= 37%).
3. Strict Invariant & Full-Precision Normalization:
   - Stage 2 clusters are strictly restricted to realized district (0 mismatches).
   - Probability vectors normalized in float64 with sum error <= 1e-9.
"""
...
from ml.data.generate_delhi_v5_dataset import (
    DELHI_CLUSTERS_V5 as DELHI_CLUSTERS_V6_2,
    ALL_11_DISTRICTS,
    DISTRICT_ADJACENCY,
    FRAUD_PROFILES,
    haversine_km
...
CLUSTER_BY_ID: Dict[int, Dict[str, Any]] = {c["id"]: c for c in DELHI_CLUSTERS_V6_2}
ALL_CLUSTER_IDS: List[int] = [c["id"] for c in DELHI_CLUSTERS_V6_2]
...
# Fraud type preferred roles (decoupled from individual cluster IDs)
...
class DelhiV62DatasetGenerator:
    """
    Generator for Delhi V6.2 controlled synthetic cybercrime dataset.
    """

    def __init__(self, seed: int = 38184, total_cases: int = 15000):
        self.seed = seed
    ...
    def _init_sub_prototypes(self) -> List[Dict[str, Any]]:
        """Sets up 54 latent sub-prototypes with coherent spatial routes and role preferences."""
        protos = []
        proto_idx = 1
        for arch_name, _ in ARCHETYPE_SPECS:
            for sub_i in range(1, 7):
                ...
                protos.append({
                    "sub_prototype_id": f"SUB-PROTO-V62-{proto_idx:02d}",
                    "archetype": arch_name,
                    "sub_index": sub_i,
    ...
    def generate_case(self, case_num: int, base_time: datetime.datetime) -> Dict[str, Any]:
        """Generates a single comprehensive V6.2 case record with coherent route and consensus observables."""
        ...
        dists_to_v = [haversine_km(v_lat, v_lon, c["lat"], c["lon"]) for c in DELHI_CLUSTERS_V6_2]
        nearest_origin_cid = DELHI_CLUSTERS_V6_2[int(np.argmin(dists_to_v))]["id"]
        ...
        comm_hub_ref = CLUSTER_BY_ID[7] # Connaught Place
        comm_proximity = round(haversine_km(v_lat, v_lon, comm_hub_ref["lat"], comm_hub_ref["lon"]), 2)

        # 11 New V6.2 Route Coherence & Consensus Observables
        ...
        # =========================================================================
        dist_weights = np.ones(len(ALL_11_DISTRICTS), dtype=np.float64) * 0.08 # Neutral baseline

        consensus_boost = 1.0 + 1.2 * district_evidence_consensus_score

        for i, d in enumerate(ALL_11_DISTRICTS):
            # Channel A: Downstream account geography
            if d == dom_d:
                dist_weights[i] += 4.2 * dom_share * geo_consistency * consensus_boost
            if d == maj_down_d:
                dist_weights[i] += 3.6 * majority_downstream_share * downstream_direction_strength

            # Channel B: Terminal mule geography
            if d == t_dist:
                dist_weights[i] += 3.8 * terminal_flow_alignment * consensus_boost

            # Channel C: Money-flow trajectory & corridors
            if d == dominant_flow_district:
                dist_weights[i] += 2.4 * downstream_route_consistency
            if d in DISTRICT_ADJACENCY[maj_down_d] or d in DISTRICT_ADJACENCY[t_dist]:
                dist_weights[i] += 1.2 * (1.0 - backtrack_ratio)

            # Channel D: Origin & behavioral context
            if d == v_dist:
                if arch_name in ["LOCAL_RAPID_CASHOUT", "NIGHTTIME_ATM_CASHOUT"]:
                    dist_weights[i] += 5.2
                elif arch_name == "CROSS_DISTRICT_MULE_CHAIN":
                    dist_weights[i] += 0.15 # Strong cross-district push
                else:
                    dist_weights[i] += 1.0

        # Special archetype corridor weighting
        if arch_name == "TRANSPORT_HUB_MOVEMENT":
            for d in ["CENTRAL", "NEW_DELHI", "NORTH", "EAST", "SHAHDARA"]:
                dist_weights[ALL_11_DISTRICTS.index(d)] += 2.8
        ...
        # =========================================================================
        # STAGE 2: SAMPLE CLUSTER INSIDE REALIZED DISTRICT (District-Local Roles)
        # =========================================================================
        cand_clusters = self.clusters_by_district[realized_district]
        n_cands = len(cand_clusters)
        cluster_weights = np.ones(n_cands, dtype=np.float64) * 0.40 # Baseline intra-district mass
        ...
        for i, c in enumerate(cand_clusters):
            ...
            # 1. Sub-prototype functional role match
            matched_proto_roles = [r for r in preferred_roles if r in c_roles]
            if matched_proto_roles:
                cluster_weights[i] += 4.5 * len(matched_proto_roles)

            # 2. Fraud type functional role match (decoupled from individual cluster IDs)
            matched_fraud_roles = [r for r in fraud_roles if r in c_roles]
            if matched_fraud_roles:
                cluster_weights[i] += 2.2 * len(matched_fraud_roles)

            # 3. ATM Density Role
            if "HIGH_DENSITY_ATM_CORRIDOR" in c_roles and arch_name in ["HIGH_VALUE_DELAYED_CASHOUT", "NIGHTTIME_ATM_CASHOUT"]:
                cluster_weights[i] += 3.0

            # 4. Syndicate corridor role (strictly within preferred syndicate clusters)
            if arch_name == "RECURRING_SYNDICATE_CORRIDOR":
                if cid in syn["preferred_clusters"]:
                    cluster_weights[i] += 5.0

            # 5. Global popularity cap (capped at <= 12% contribution)
            hist_pop_contrib = min(0.60, 0.12 * (c["atm_density"] / 10.0))
            cluster_weights[i] += hist_pop_contrib
        ...
        # Family IDs
        syn_family = syn["id"] if arch_name == "RECURRING_SYNDICATE_CORRIDOR" else f"SYN-V62-{(case_num % 100) + 1:03d}"
        mule_family = f"MULE-V62-{(case_num % 150) + 1:03d}"

        return {
            "case_id": f"CMP-V62-{case_num:06d}",
            "amount": amount,
    ...
    def generate_dataset(self) -> pd.DataFrame:
        ...
        print(f"Generating {self.total_cases} cases with seed {self.seed} (V6.2 Two-Stage Model)...")
        ...
        df["case_id"] = [f"CMP-V62-{k+1:06d}" for k in range(self.total_cases)]
...
def main():
    parser = argparse.ArgumentParser(description="Generate Delhi V6.2 Controlled Synthetic Dataset")
    parser.add_argument("--cases", type=int, default=15000, help="Total cases (default: 15000)")
    parser.add_argument("--seed", type=int, default=38184, help="RNG seed (default: 38184)")
    parser.add_argument("--output", type=str, default=None, help="Output path for csv.gz")
    args = parser.parse_args()

    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    out_path = args.output or os.path.join(base_dir, "ml", "data", "delhi_v6_2_cases.csv.gz")

    gen = DelhiV62DatasetGenerator(seed=args.seed, total_cases=args.cases)
